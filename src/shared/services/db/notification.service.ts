...
class NotificationService {
  public async getNotifications(userId: string): Promise<INotificationDocument[]> {
    // -------------------------------------------------------------------------
    // TODO: ⚠️ PERFORMANCE WARNING (Scalability)
    // We are fetching ALL notifications for a user just to display ONE new notification via Socket.
    //
    // Problem:
    // If a user has 5,000 old notifications, every 'Like' or 'Comment' will fetch
    // and return 5,001 documents from DB. This is heavy on bandwidth and DB CPU.
    //
    // FUTURE FIX:
    // Create a method `getSingleNotification(notificationId)` to fetch only the
    // newly created notification for the Socket event.
    // -------------------------------------------------------------------------
