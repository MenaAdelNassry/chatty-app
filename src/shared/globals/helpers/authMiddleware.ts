...
export class AuthMiddleware {
  public async verifyUser(req: Request, _res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      const authUser: IAuthDocument | null = await authService.getAuthUserByEmail(payload.email);
  ...
  public checkAuthentication(req: Request, _res: Response, next: NextFunction): void {
    ...
    if (!req.currentUser.emailVerified) {
      throw new NotAuthorizedError('Please verify your email address to perform this action.');
    }
    next();
  }
}
